        ...
        public void UpdatePosition(Board board)
        {
            for (int rank = 0; rank < 8; rank++)
            {
                for (int file = 0; file < 8; file++)
                {
                    ...
                    squarePieceRenderers[file, rank].sprite = pieceTheme.GetPieceSprite(piece, coord);
                    squarePieceRenderers[file, rank].transform.position = PositionFromCoord(file, rank, pieceDepth);
                }
            }
        }

        public void UpdateBLindChess(Coord coord, int index)
        {
            squarePieceRenderers[coord.fileIndex, coord.rankIndex].sprite = pieceTheme.Dut(index);
        }
        ...
        void CreateBoardUI()
        {
            ...
            for (int rank = 0; rank < 8; rank++)
            {
                for (int file = 0; file < 8; file++)
                {
                    ...
                    // pieceRenderer.transform.localScale = Vector3.one * 100 / (2000 / 6f);   //TODO:Scale Chess
