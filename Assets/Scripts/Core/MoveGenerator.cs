...
namespace Chess
{
    ...
    public class MoveGenerator
    {
        ...
        public List<Move> GenerateMoves(Board board, bool includeQuietMoves = true)
        {
            ...
            // Debug.Log($"is white to move: {isWhiteToMove}");
            return moves;
        }
        ...
        void GenarateQueenMove()
        {
            if (!BlindChessController.instance.isPlayOffline)
            {
                ...
                for (int i = 0; i < queens.Count; i++)
                {
                    ...
                    bool isFirstMove = isWhiteToMove
                        ? BlindChessController.instance.isWhiteQueenFirstMove
                        : BlindChessController.instance.isBlackQueenFirstMove;
                    ...
                    if (isFirstMove)
                    {
                        // Blind Chess mode: can only move forward by one square
                        int targetSquare = startSquare + (friendlyColour == Piece.White ? 8 : -8);
                        int targetSquarePiece = board.Square[targetSquare];
                        bool isCapture = Piece.IsColour(targetSquarePiece, opponentColour);

                        if ((genQuiets || isCapture) && !Piece.IsColour(targetSquarePiece, friendlyColour))
                        {
        ...
        void GenarateBishopMove()
        {
            if (!BlindChessController.instance.isPlayOffline)
            {
                var currentPiece = PlayerPrefs.GetString("CurrentPiece", string.Empty);
                var pieceName = "";
                var pieceIndex = 1;
                if (!string.IsNullOrEmpty(currentPiece))
                {
                    string[] tmp = currentPiece.Split('_');
                    pieceName = tmp[0];
                    pieceIndex = (int)Mathf.Clamp01(int.Parse(tmp[1]) - 1);

                    if (pieceName.Contains("Bishop"))
                    {
                        pieceIndex = int.Parse(tmp[1]);
                    }
                }
                ...
                for (int i = 0; i < bishops.Count; i++)
                {
                    ...
                    if (!BlindChessController.instance.isBlindChess /*|| bishops.Count <= 1*/)
                    {
                        isFirstMove = false;
                    }
                    if (isFirstMove)
        ...
        void GenarateRookMove()
        {
            if (!BlindChessController.instance.isPlayOffline)
            {
                ...
                for (int i = 0; i < rooks.Count; i++)
                {
                    ...
                    bool isFirstMove = pieceIndex != -1 && BlindChessController.instance.PieceSelected[pieceIndex];
                    if (!BlindChessController.instance.isBlindChess /*|| rooks.Count <= 1*/)
                    {
                        isFirstMove = false;
                    }
        ...
        {
            //todo: TINH phần này lấy data đã lưu ra để check
            // Debug.LogError($"Current Piece: {PlayerPrefs.GetString("CurrentPiece", string.Empty)}");
            if (!BlindChessController.instance.isPlayOffline)
            {
                var currentPiece = PlayerPrefs.GetString("CurrentPiece", string.Empty);
                var pieceName = "";
                ...
                for (int i = 0; i < myKnights.Count; i++)
                {
                    ...
                    bool isFirstMove = pieceIndex != -1 && BlindChessController.instance.PieceSelected[pieceIndex];
                    if (!BlindChessController.instance.isBlindChess /*|| myKnights.Count < 1*/)
                    {
                        isFirstMove = false;
                    }
                    ...
                    if (isFirstMove)
                    {
                        // Blind Chess mode: Knight can only move forward by one square
                        int targetSquare = startSquare + (friendlyColour == Piece.White ? 8 : -8);
                        int targetSquarePiece = board.Square[targetSquare];
                        bool isCapture = Piece.IsColour(targetSquarePiece, opponentColour);
                ...
                for (int i = 0; i < myPawns.Count; i++)
                {
                    ...
                    if (genQuiets)
                    {
                        ...
                        if (board.Square[squareOneForward] == Piece.None)
                        {
                            ...
                            if (!IsPinned(startSquare) ||
                                IsMovingAlongRay(pawnOffset, startSquare, friendlyKingSquare))
                            {
                    ...
                    for (int j = 0; j < 2; j++)
                    {
                        ...
                        if (numSquaresToEdge[startSquare][pawnAttackDirections[friendlyColourIndex][j]] > 0)
                        {
                            ...
                            if (IsPinned(startSquare) &&
                                !IsMovingAlongRay(pawnCaptureDir, friendlyKingSquare, startSquare))
                            {
                                continue;
                            }
